"""
AI Work Email Automation Agent
Full integration with Gmail Watcher and Email Queue Processing system
"""
...
def extract_job_details_from_email(email_body: str) -> Dict[str, Any]:
    """Extract job-related details from email content as per requirements."""
    details = {
        'company_name': '',
        'position': '',
        'employment_type': '',
...
def assign_priority(email_body: str, position: str, subject: str) -> str:
    """Assign priority level as per requirements:
    HIGH → job offer / interview
    MEDIUM → freelance or recruiter message
    LOW → hiring newsletter
    """
...
def generate_professional_response(email_info: Dict[str, Any], job_details: Dict[str, Any]) -> str:
    """Generate a professional response that meets requirements:
    - Thanks the sender
    - Shows interest
    - Asks clarification if role or salary missing
    ...
    # Opening with contact person if available
    contact_person = job_details.get('contact_person', 'Hiring Manager')
    response_parts.append(f"Dear {contact_person},")
    response_parts.append("")

    # Thank you and interest statement
    company_name = job_details.get('company_name', 'your company')
    position = job_details.get('position', 'position')
    response_parts.append(f"Thank you for reaching out regarding the {position} opportunity at {company_name}.")
    response_parts.append("")
    ...
    # General interest statement if we have the required info
    if not missing_info_requests:  # Only add if we didn't already ask for details
        response_parts.append(f"I would appreciate the opportunity to discuss this role further and explore how I can contribute to {company_name}.")
...
def save_to_approval_queue(company: str, position: str, priority: str, draft_response: str):
    """Save draft response to approval queue as per requirements."""
    approval_data = {
        "company": company,
        "position": position,
        "priority": priority,
    ...
    try:
        ...
        for email in emails:
            try:
                ...
                # Generate professional response as per requirements
                draft_response = generate_professional_response(email, job_details)

                # Save to approval queue (instead of sending automatically) as per requirements
                save_to_approval_queue(
...
def process_general_emails():
    """Process general emails from the general queue."""
...
def monitor_email_queues():
    """Monitor both email queues as per integration requirements."""
    logger.info("AI Work Email Automation Agent started")
    ...
    try:
        while True:
            ...
            # Check memory usage before processing (memory safety failsafe)
            memory_percent = get_memory_usage()
            ...
            # Process general emails based on interval
            ...
            # Sleep for work email interval (180 seconds) as per requirements
            time.sleep(WORK_CHECK_INTERVAL)

    except KeyboardInterrupt:
        logger.info("AI Work Email Automation Agent stopped by user")
    except Exception as e:
        logger.error(f"Error in main loop: {e}")
        raise

if __name__ == "__main__":
    monitor_email_queues()